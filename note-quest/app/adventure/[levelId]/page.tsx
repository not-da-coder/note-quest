"use client";
// app/adventure/[levelId]/page.tsx — Individual Level
...
import { accuracy } from "@/lib/game/questionEngine";
...
export default function LevelPage() {
  ...
  const startGame = useCallback(() => {
    if (!level) return;
    const p = loadProgress();
    const qs = generateQuestions(level.notesWorld, level.questionCount, p.noteMemory);
    setQuestions(qs);
    setMemory(p.noteMemory);
    setScreen("playing");
    setResult(null);
  ...
  if (!level || !world) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 px-5">
        ...
        <Link href="/adventure" className="text-violet font-body text-sm">
          ← Back to Map
        </Link>
      </div>
    );
  ...
  if (!unlocked) {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen gap-4 px-5 text-center">
        ...
        <Link
          href="/adventure"
          className="mt-4 px-6 py-3 rounded-2xl bg-violet text-white font-body font-bold"
        >
          Back to Map
        </Link>
      </div>
  ...
  if (screen === "intro") {
    return (
      <div className="flex flex-col items-center justify-center min-h-screen px-5 gap-7 animate-slide-up">
        ...
        <div className="w-full bg-white/70 border border-mist rounded-2xl p-5 text-center space-y-1">
          ...
          <p className="text-sm font-body text-ink/60 mt-2">{world.hint}</p>
        </div>
    ...
    return (
      <div className="flex flex-col min-h-screen pt-8 pb-6 gap-4">
        ...
        <GameEngine
          ...
          // No timer in adventure mode (learner-paced)
        />
      </div>
    );
  }
